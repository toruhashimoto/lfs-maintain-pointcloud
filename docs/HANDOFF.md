...
- `--sh-degree` 3 → 2 / 1 / 0: **悪化**。最終 PSNR / SSIM とも次数に対して単調

| sh_degree | ピーク(7,000) | 最終 PSNR | 最終 SSIM |
|---|---|---|---|
| 0 | 17.069 | 15.989 | 0.6397 |
| 1 | 17.243 | 16.092 | 0.6405 |
| 2 | 17.185 | 16.190 | 0.6455 |
| **3（既定, n=2）** | **17.383** | **16.524** | **0.6498** |

既定の 3 が最良。この被写体には視点依存カラーが実際に必要で、削ると表現力不足になる
（ピーク時点で既に劣る＝過学習ではない）。sh=0/1 の複製は回していないが、4指標すべてで
順序が一致し、SSIM で 3倍ノイズ分離れているため結論は動かないと判断して打ち切った。
